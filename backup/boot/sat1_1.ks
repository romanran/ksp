...
ON AG6{
	DELETE("1:/boot/sat1_1.ks").
	COPYPATH("0:/boot/sat1_1.ks", "1:/").
	REBOOT.
	CLEAR.
	RUNONCEPATH("1:/sat1_1.ks").
}
ON AG1{
	SET s TO TRUE.
}
	...
	IF doneStaging=false{
		IF STAGE:LIQUIDFUEL < 1 AND stg_res:HASKEY("LIQUIDFUEL"){
			//FOR eng IN ship_engines{
				//eng:SHUTDOWN.
			//}
			chronos:CONNECTION:SENDMESSAGE(1).
